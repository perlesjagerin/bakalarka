...
\thanks{Děkuji vedoucímu práce Mgr. Jiřímu Zacpalovi, Ph.D. za odborné vedení, konstruktivní připomínky a čas věnovaný konzultacím. Zvláštní dík patří mé rodině za neustálou podporu a povzbuzení v průběhu celého studia.}
...
Pro akce zdarma (ticketPrice = 0) je rezervace okamžitě potvrzena se statusem CONFIRMED bez platebního procesu. Uživatel může své rezervace spravovat na stránce "Moje rezervace", kde může rezervaci zrušit (refundace pro placené rezervace) nebo stáhnout PDF vstupenku s QR kódem.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\textwidth]{graphics/reservation-flow.png}
    \caption{Sekvenční diagram procesu rezervace a platby}
    \label{fig:reservationflow}
\end{figure}

Obrázek~\ref{fig:reservationflow} znázorňuje kompletní flow rezervace vstupenek. Proces začíná výběrem akce, pokračuje vytvořením rezervace v databázové transakci (s kontrolou dostupnosti a atomickým snížením počtu vstupenek), následuje platba přes Stripe Payment Intent pro placené akce nebo okamžité potvrzení pro akce zdarma, a končí webhook potvrzením platby a odesláním emailové notifikace.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth]{graphics/reservation-states.png}
    \caption{Stavový diagram rezervace}
    \label{fig:reservationstates}
\end{figure}

Obrázek~\ref{fig:reservationstates} ukazuje přechody mezi stavy rezervace. Placené akce začínají ve stavu PENDING (čeká na platbu) a přechází do stavu PAID po úspěšné platbě. Akce zdarma jsou okamžitě potvrzeny se statusem CONFIRMED. Rezervace může končit ve stavu CANCELLED (zrušeno bez platby) nebo REFUNDED (zrušeno s vrácením peněz). Refundace může nastat z důvodu schválené reklamace, zrušení uživatelem nebo zrušení celé akce organizátorem.
...
\begin{enumerate}
    ...
    \item Aktualizuje stav rezervace na REFUNDED (pro placené akce) nebo CANCELLED (pro akce zdarma)
...
Hesla jsou bezpečně hashována pomocí \textbf{bcryptjs} s salt rounds nastaveným na 10. Middleware \texttt{authenticate} chrání všechny chráněné API endpointy a automaticky ověřuje JWT tokeny. Proces zahrnuje registraci s hashováním hesla, přihlášení s generováním JWT tokenů, autorizované API požadavky s ověřováním tokenu, refresh mechanismus pro obnovení access tokenu a bezpečné odhlášení se smazáním tokenů.
...
Každá potvrzená rezervace (status PAID nebo CONFIRMED) umožňuje stažení PDF vstupenky, která obsahuje:
\begin{itemize}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\textwidth]{graphics/database-model.png}
...
Následně se aktualizuje status rezervace na REFUNDED (pro placené akce) nebo CANCELLED (pro akce zdarma). Vrátí se vstupenky do akce zvýšením pole availableTickets o počet vstupenek z rezervace. Nakonec se odešle email uživateli s potvrzením zrušení.

Pro akce zdarma (status CONFIRMED) se Stripe refund přeskočí a provádí se pouze zrušení rezervace se statusem CANCELLED a vrácení vstupenek.
